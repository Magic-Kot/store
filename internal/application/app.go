...
import (
	...
	"github.com/Magic-Kot/store/internal/service/referral"
	"github.com/Magic-Kot/store/internal/service/user"
...
func (app *App) Run() error {
	...
	logger.Info().Msg(fmt.Sprintf("connecting to postgres", "database", app.cfg.Postgres.DSN))
	...
	g.Go(func() error {
		go func() {
			...
			if err := app.httpServer.Shutdown(ctx); err != nil {
				//contextx.LoggerFromContextOrDefault(ctx).Error("httpServer.Shutdown", "error", err)
			}
		}()
...
func (app *App) newHTTPServer(ctx context.Context) *http.Server {
	...
	router.Use(
		...
		//middleware.RealIP,
		//middlewarex.TraceID,
		//middlewarex.JWTAuth(middlewarex.NewHeaderAuthorizationBearerTokenFinder(), component.NewAccessTokenParser(app.cfg.JWT.PublicKey)),
		//middlewarex.Logger,
		//middlewarex.RequestLogging(masker.NewNopSensitiveDataMasker(), app.cfg.Log.FieldMaxLen),
		//middlewarex.ResponseLogging(masker.NewNopSensitiveDataMasker(), app.cfg.Log.FieldMaxLen),
...
func ConnectRedis(
	...
	slog.Info(
		"redis connected",
		"address", address,
		"database-number", databaseNumber,
	)
