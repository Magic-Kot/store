...
func replyError(ctx context.Context, w http.ResponseWriter, err error) {
	...
	response := gateway_api.ErrorModel{ //nolint:exhaustruct
		Code:    gateway_api.ErrorCodeType(failure.Code(err).String()),
	...
	case failure.IsInvalidArgumentError(err):
		...
		if failure.Code(err) == "" {
			response.Code = gateway_api.ErrorCodeType(errcodes.ValidationError.String())
		}
	...
	case failure.IsNotFoundError(err):
		...
		if response.Code == "" {
			response.Code = gateway_api.ErrorCodeType(errcodes.NotFound.String())
		}
	...
	case failure.IsForbiddenError(err):
		...
		if response.Code == "" {
			response.Code = gateway_api.ErrorCodeType(errcodes.Forbidden.String())
		}
	...
	default:
		...
		if response.Code == "" {
			response.Code = gateway_api.ErrorCodeType(errcodes.InternalServerError.String())
		}
