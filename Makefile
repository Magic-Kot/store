...
goose-up:
	...
	-goose -v -dir $(MIGRATION_DIR)/$(db) postgres "postgresql://gfk:gfk@localhost:5432/$(db)?sslmode=disable" up
	goose -v -dir $(MIGRATION_DIR)/$(db) postgres "postgresql://gfk:gfk@localhost:5432/$(db)?sslmode=disable" status
